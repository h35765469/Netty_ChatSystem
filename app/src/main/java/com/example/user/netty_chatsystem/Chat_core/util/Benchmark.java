 ...
 */
package com.example.user.netty_chatsystem.Chat_core.util;
...
public class Benchmark {
    ...
    public static void end(String tag) {
        ...
        if (!entry.tag.equals(tag)) {
            //logger.warn("Benchmark Not Match, tag spell mistake or forgot Benchmark.end(tag) invoke at somewhere ??");
            return;
        }

        entry.end = new Date();
        long used = entry.end.getTime() - entry.start.getTime();
        //logger.debug("Benchmark [" + entry.tag + " ] - Used: " + (used) + " ms. ");
    }
